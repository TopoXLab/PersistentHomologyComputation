...
public:	
	...
	void initList(vector< int > *birth_list, vector< MatrixListType > *cell2v_list, int d, bool verbose=false)
	{
		birth_list->assign(cellCount[d], -1);		  
		cell2v_list->assign(cellCount[d], vector<int>());		  
		generateCellLists(*birth_list, *cell2v_list, d, verbose);
	}
...
private:
	...
	void generateCellLists(vector<int> &birth_list, vector<MatrixListType> &cell2v_list, int d, bool verbose=false)
	{
		if (verbose) OUTPUT_MSG("start explicit cell generation");
		...
		if (verbose) cout << "Dimension " << d << " contains " <<(int) cell2v_list.size() << " cells" << endl;
		...
		if (verbose) OUTPUT_MSG("end explicit cell generation");
	}
};
